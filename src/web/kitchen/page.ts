...
<body>
  ...
  <script>
    ...
    const state={event:null,orders:[],products:[],activeTab:'pending',baselineEventId:null,voiceSettings:readVoiceSettings(),reminderLedger:readReminderLedger()};
    ...
    function clearForNoEvent(){state.event=null;state.orders=[];state.products=[];state.baselineEventId=null;$('event').textContent='目前沒有營業中的場次';$('page-state').textContent='請先至後台開啟場次，Kitchen 才會顯示中央訂單。';$('page-state').className='page-state';render()}
    async function load(){try{const event=await api('/api/events/current');sync.setEventId(event?.eventId);if(!event){clearForNoEvent();return}const [orders,products]=await Promise.all([api('/api/events/'+event.eventId+'/orders'),api('/api/events/current/products')]),isBaseline=state.baselineEventId!==event.eventId;state.event=event;state.orders=orders;state.products=products;state.baselineEventId=event.eventId;const status=String(event.status||'').toUpperCase();$('event').innerHTML=esc(event.displayName)+' · '+esc(event.eventCode)+' <span class="event-status '+esc(event.status)+'">'+esc(status)+'</span>';$('page-state').textContent=event.status==='paused'?'場次暫停中，Kitchen 僅處理現有訂單。':'中央訂單已同步。';$('page-state').className='page-state';render();processAutomaticVoice(isBaseline)}catch(error){$('page-state').textContent=(navigator.onLine?'中央資料讀取失敗：':'目前離線，無法讀取中央訂單：')+(error?.message||'未知錯誤');$('page-state').className='page-state error';throw error}}
    ...
    sync.registerLoad(load);renderVoiceSettings();setInterval(()=>processAutomaticVoice(false),30000);
  </script>
</body>
</html>`;
}
