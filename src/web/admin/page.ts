      ...
      <div class="stack">
        <section>
          ...
          <form id="category-form">
            ...
            <label>品項分類名稱<input id="category-name" required placeholder="荒島飯盒"></label>
            <div class="row">
            ...
            <p id="category-code-display" class="meta">系統代碼會在儲存後自動產生</p>
            <button type="submit">儲存品項分類</button>
          </form>
...
function displayCategoryCode(code){return /^cat-[0-9]{4}$/.test(code)?code.toUpperCase():code}
...
function resetCategory(){$('category-form').reset();$('category-id').value='';$('category-sort').value='0';$('category-active').value='true';$('category-code-display').textContent='系統代碼會在儲存後自動產生'}
...
function renderCategories(){const target=$('categories');target.innerHTML=state.categories.map(category=>'<div class="item '+(category.isActive?'':'inactive')+'"><div><strong>'+esc(category.displayName)+'</strong> <span class="status '+(category.isActive?'ok':'inactive')+'">'+(category.isActive?'啟用':'停用')+'</span><div class="meta">系統代碼 '+esc(displayCategoryCode(category.code))+' · 排序 '+category.sortOrder+'</div></div><button class="secondary" data-category="'+category.categoryId+'">編輯</button></div>').join('')||'<div class="empty">目前沒有品項分類</div>';$('product-category').innerHTML='<option value="">選擇分類</option>'+activeCategories().map(category=>'<option value="'+category.categoryId+'">'+esc(category.displayName)+'</option>').join('')}
...
$('category-form').addEventListener('submit',async event=>{event.preventDefault();try{const id=$('category-id').value,body={displayName:$('category-name').value,sortOrder:Number($('category-sort').value),isActive:$('category-active').value==='true'};await api(id?'/api/admin/categories/'+id:'/api/admin/categories',{method:id?'PATCH':'POST',body:JSON.stringify(body)});resetCategory();await load();message('品項分類已儲存')}catch(error){message(error.message,true)}});
