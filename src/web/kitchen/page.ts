...
<body>
  ...
  <script>
    ...
    function servedSummary(orders){return orders.map(o=>'<article class="card served"><div class="order-no">'+esc(o.orderNumber)+(o.customerName?' · '+esc(o.customerName):'')+'</div><div class="items">'+((o.items||[]).map(i=>'<div>'+esc(i.posName)+' x'+i.quantity+'</div>').join('')||'<div>沒有品項</div>')+'</div><div class="meta"><span class="tag">已出餐</span>'+(o.servedAt?'<span class="tag">'+esc(new Date(o.servedAt).toLocaleTimeString('zh-TW',{hour:'2-digit',minute:'2-digit'}))+'</span>':'')+'</div><div class="actions"><button type="button" data-revert-production="'+o.orderId+'">退回上一步</button></div></article>').join('')||'<p class="empty">目前沒有可退回的已出餐訂單。</p>'}
    async function load(){const e=await api('/api/events/current');sync.setEventId(e?.eventId);if(!e){$('#event').textContent='目前沒有 OPEN Event';$('#pending').innerHTML='<p class="empty">目前沒有要出的單。</p>';$('#preparing').innerHTML='<p class="empty">目前沒有需要製作的餐點。</p>';$('#summary').textContent='沒有開啟中的場次';$('#pending-count').textContent='0';$('#preparing-count').textContent='0';$('#ready-count').textContent='0';return}const orders=await api('/api/events/'+e.eventId+'/orders'),active=orders.filter(o=>o.orderStatus==='confirmed'&&o.productionStatus!=='served'),served=orders.filter(o=>o.orderStatus==='confirmed'&&o.productionStatus==='served');$('#event').textContent=e.displayName+' · '+e.eventCode;$('#pending').innerHTML=active.map(card).join('')||'<p class="empty">目前 30 分鐘內沒有要出的單。</p>';$('#preparing').innerHTML=prepSummary(active);$('#ready').innerHTML=servedSummary(served);$('#pending-count').textContent=String(active.length);$('#preparing-count').textContent=String(active.reduce((sum,order)=>sum+(order.items||[]).reduce((itemSum,item)=>itemSum+item.quantity,0),0));$('#ready-count').textContent=String(served.length);$('#summary').textContent='剩餘總份數由 POS 顯示；目前要出的單 '+active.length+'，可取餐／已出餐 '+served.length}
    document.addEventListener('click',async e=>{const revert=e.target.closest('button[data-revert-production]');if(revert){if(!confirm('確定將此訂單退回前一個製作狀態？\\n此操作不會取消訂單、退款或修改庫存。'))return;revert.disabled=true;try{const deviceId=new URLSearchParams(location.search).get('device')||'Kitchen';await api('/api/orders/'+revert.dataset.revertProduction+'/production/revert-completion',{method:'POST',body:JSON.stringify({confirmed:true,reason:'accidental_completion',operator:'kitchen',deviceId})});await load()}catch(err){alert(err instanceof TypeError?'無法連線，退回操作未送達中央':err.message)}finally{revert.disabled=false}return}const b=e.target.closest('button[data-status]');if(!b)return;b.disabled=true;try{await api('/api/orders/'+b.dataset.id+'/status',{method:'PATCH',body:JSON.stringify({status:b.dataset.status,operator:'kitchen'})});await load()}catch(err){alert(err instanceof TypeError?'無法連線，請確認中央服務':err.message)}finally{b.disabled=false}});
