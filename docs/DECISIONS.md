...
- **DECISIONS #091 - Production Runtime Configuration and Secure Deployment Boundary**
  ...
  - **Protected boundary and exact scope**:
    - PR-PLATFORM-002 fixes exactly sixteen implementation paths: environment/sample and command metadata; runtime/migration/server/access-control/authentication configuration; one preflight script; non-secret systemd and Nginx templates; production/security documentation; focused runtime, server, authentication, and Architecture Guard tests.
    ...
    - The Architecture Guard must substantively classify Production Runtime / Secure Deployment responsibility, permit only the exact sixteen paths, and reject a simulated unauthorized seventeenth responsibility path through the same classifier.
