...
const state={filter:'all',collection:[],selectedIngredientId:null,detail:null,listRequest:0,detailRequest:0,commandPending:false};
const byId=id=>document.getElementById(id);
class RequestFailure extends Error{constructor(status,code,message){super(message);this.name='RequestFailure';this.status=status;this.code=code}}
...
function showNotice(message,kind){const notice=byId('notice');notice.textContent=message;notice.className='notice'+(kind?' '+kind:'')}
...
async function api(path,options){let response;try{response=await fetch(path,{headers:options&&options.body?{'content-type':'application/json'}:undefined,...options})}catch{throw new RequestFailure(0,'NETWORK_OFFLINE','無法連線，請確認網路後再試。')}let body;try{body=await response.json()}catch{throw new RequestFailure(response.status,'UNUSABLE_RESPONSE','伺服器回應格式無法使用。')}if(!response.ok){const error=body&&body.error?body.error:{};throw new RequestFailure(response.status,String(error.code||'UNKNOWN_ERROR'),String(error.message||'請求失敗。'))}if(!body||body.ok!==true)throw new RequestFailure(response.status,'UNUSABLE_RESPONSE','伺服器回應格式無法使用。');return body.data}
...
async function loadCollection(){const request=++state.listRequest;byId('collection-loading').hidden=false;try{const data=await api(API_ROOT+'?lifecycle='+encodeURIComponent(state.filter));if(request!==state.listRequest)return;state.collection=Array.isArray(data)?data:[];renderCollection()}catch(error){if(request===state.listRequest)showNotice(error instanceof RequestFailure?error.message:'清單載入失敗。','error')}finally{if(request===state.listRequest)byId('collection-loading').hidden=true}}
async function loadDetail(ingredientId){const request=++state.detailRequest;state.selectedIngredientId=ingredientId;renderCollection();byId('detail-loading').hidden=false;try{const detail=await api(API_ROOT+'/'+encodeURIComponent(ingredientId));if(request!==state.detailRequest)return;state.detail=detail;renderDetail();showNotice('已載入「'+detail.name+'」的最新資料。','')}catch(error){if(request!==state.detailRequest)return;if(error instanceof RequestFailure&&error.status===404){state.selectedIngredientId=null;state.detail=null;renderDetail();await loadCollection();showNotice('食材已不存在，清單已重新整理。','warning')}else{showNotice(error instanceof RequestFailure?error.message:'詳細資料載入失敗。','error')}}finally{if(request===state.detailRequest)byId('detail-loading').hidden=true}}
async function refreshSelected(){const selected=state.selectedIngredientId;await loadCollection();if(selected&&state.selectedIngredientId===selected)await loadDetail(selected)}
async function handleFailure(error,operation){if(!(error instanceof RequestFailure)){showNotice('操作失敗，請稍後再試。','error');return}if(error.status===409){if(error.code==='CANONICAL_INGREDIENT_VERSION_CONFLICT'){await refreshSelected();showNotice('版本已變更，已重新載入最新資料；請重新檢視後再決定，不會自動重送。','warning');return}if(error.code==='CANONICAL_INGREDIENT_ALREADY_ARCHIVED'){await refreshSelected();showNotice('此食材已封存，已重新載入唯讀資料。','warning');return}if(error.code==='CANONICAL_INGREDIENT_ARCHIVED_RENAME_REJECTED'){await refreshSelected();showNotice('已封存食材不能更名，已重新載入唯讀資料。','warning');return}if(error.code==='INVALID_CANONICAL_INGREDIENT_TRANSITION'){await refreshSelected();showNotice('目前狀態不允許'+operation+'，請重新檢視最新資料。','warning');return}showNotice('操作發生衝突，請重新整理後再試。','warning');return}if(error.status===404){state.selectedIngredientId=null;state.detail=null;renderDetail();await loadCollection();showNotice('食材已不存在，清單已重新整理。','warning');return}if(error.status===422){showNotice(error.message,'error');return}if(error.status===0){showNotice(error.message,'error');return}showNotice('服務暫時無法完成操作，輸入內容已保留。','error')}
byId('lifecycle-filter').addEventListener('change',event=>{state.filter=event.currentTarget.value;void loadCollection()});
byId('refresh-list').addEventListener('click',()=>{void refreshSelected()});
byId('rename-form').addEventListener('submit',async event=>{event.preventDefault();if(state.commandPending||!state.detail)return;setCommandPending(true);try{const result=await api(API_ROOT+'/'+encodeURIComponent(state.detail.ingredientId)+'/rename',{method:'POST',body:JSON.stringify({newName:requiredValue('rename-name','新名稱'),expectedVersion:state.detail.aggregateVersion,actor:requiredValue('rename-actor','操作者'),occurredAt:localTimeToUtc('rename-occurred-at'),reason:requiredValue('rename-reason','原因')})});state.detail=result.ingredient;renderDetail();renderDuplicateWarning(result.warnings);await loadCollection();showNotice('更名已完成。','')}catch(error){await handleFailure(error,'更名')}finally{setCommandPending(false)}});
byId('archive-form').addEventListener('submit',async event=>{event.preventDefault();if(state.commandPending||!state.detail)return;if(!window.confirm('確定封存「'+state.detail.name+'」？封存後仍可查閱，但不能再更名。'))return;setCommandPending(true);try{const result=await api(API_ROOT+'/'+encodeURIComponent(state.detail.ingredientId)+'/archive',{method:'POST',body:JSON.stringify({expectedVersion:state.detail.aggregateVersion,actor:requiredValue('archive-actor','操作者'),occurredAt:localTimeToUtc('archive-occurred-at'),reason:requiredValue('archive-reason','原因')})});state.detail=result.ingredient;renderDetail();await loadCollection();showNotice('食材已封存。','')}catch(error){await handleFailure(error,'封存')}finally{setCommandPending(false)}});
void loadCollection().then(()=>showNotice('食材主檔已載入。',''));
</script>
</body>
</html>`;
}
