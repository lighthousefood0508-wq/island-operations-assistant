...
function payload(eventId: string, product: any, key: string) {
  return { source: "pos", eventId, idempotencyKey: key, items: [{ productId: product.productId, productVersionId: product.productVersionId, quantity: 1, notes: null }], customerName: "Miles", customerPhoneTail: "123", paymentMethod: "CASH", notes: null };
}
...
test("two POS requests for the final sellable portion produce one success and one insufficient response", async () => {
  ...
  try {
    ...
    assert.equal(orders.body.data[0].customerPhoneTail, "123");
