...
import {
  ...
  ,AcceptedPurchaseService, AcceptedPurchaseValidationFailure, AcceptedPurchaseNotFound, AcceptedPurchaseInvalidStateFailure, AcceptedPurchaseVersionConflictFailure, AcceptedPurchaseMeasurementFailure, AcceptedPurchasePersistenceFailure, RecipeCostSnapshotService, RecipeCostSnapshotValidationFailure, RecipeCostSnapshotPersistenceFailure, CostEvidenceReadService, CostEvidenceReadNotFound, CostEvidenceReadPersistenceFailure, CostEvidenceReadValidationFailure, RecipeCostHistoryReadService, RecipeCostHistoryReadNotFound, RecipeCostHistoryReadPersistenceFailure, RecipeCostHistoryReadValidationFailure, RecipeCostAnalyticsService, RecipeCostAnalyticsReadFailure, RecipeCostAnalyticsValidationFailure
...
export class CostBackOfficeService {
  ...
  constructor(
    ...
    private readonly costEvidenceReads: Pick<CostEvidenceReadService, "listSuppliers" | "getSupplier" | "getPurchase" | "listAcceptedPurchasesForPurchase" | "getAcceptedPurchase" | "listSnapshotsForRecipe" | "getSnapshot">,
    private readonly recipeCostHistory: Pick<RecipeCostHistoryReadService, "list" | "latest" | "get">,
  ...
  getSupplier(supplierId: string) { try { return this.costEvidenceReads.getSupplier(supplierId); } catch (error) { throw this.costEvidenceReadOperation(error); } }
  getPurchase(purchaseId: string) { try { return this.costEvidenceReads.getPurchase(purchaseId); } catch (error) { throw this.costEvidenceReadOperation(error); } }
  listAcceptedPurchasesForPurchase(purchaseId: string) { try { return this.costEvidenceReads.listAcceptedPurchasesForPurchase(purchaseId); } catch (error) { throw this.costEvidenceReadOperation(error); } }
  getAcceptedPurchase(acceptedPurchaseId: string) { try { return this.costEvidenceReads.getAcceptedPurchase(acceptedPurchaseId); } catch (error) { throw this.costEvidenceReadOperation(error); } }
  listSnapshotsForRecipe(recipeId: string) { try { return this.costEvidenceReads.listSnapshotsForRecipe(recipeId); } catch (error) { throw this.costEvidenceReadOperation(error); } }
  getSnapshot(costSnapshotId: string) { try { return this.costEvidenceReads.getSnapshot(costSnapshotId); } catch (error) { throw this.costEvidenceReadOperation(error); } }
  listRecipeCostHistory(recipeId: string) { try { return this.recipeCostHistory.list(recipeId); } catch (error) { throw this.recipeCostHistoryOperation(error); } }
  getLatestRecipeCostHistory(recipeId: string) { try { return this.recipeCostHistory.latest(recipeId); } catch (error) { throw this.recipeCostHistoryOperation(error); } }
  getRecipeCostHistoryEntry(recipeId: string, costSnapshotId: string) { try { return this.recipeCostHistory.get(recipeId, costSnapshotId); } catch (error) { throw this.recipeCostHistoryOperation(error); } }
  getRecipeCostAnalytics(recipeId: string) { try { return this.recipeCostAnalytics.get(recipeId); } catch (error) { throw this.recipeCostAnalyticsOperation(error); } }
  ...
  acceptPurchase(purchaseId:string,input:JsonObject) { try { return this.acceptedPurchaseService.accept({purchaseId,expectedVersion:integer(input,"expectedVersion"),currencyCode:text(input,"currencyCode"),lines:objectArray(input,"lines").map(line=>({sourcePurchaseLineId:text(line,"sourcePurchaseLineId"),amountCoefficient:text(line,"amountCoefficient"),amountScale:integer(line,"amountScale")})),acceptedAt:text(input,"acceptedAt"),acceptedBy:text(input,"acceptedBy")}); } catch(error) { throw this.acceptedPurchaseOperation(error); } }
  ...
  private costEvidenceReadOperation(error: unknown): HttpError { if(error instanceof CostEvidenceReadValidationFailure)return new HttpError(422,"cost_evidence_read_invalid","Cost evidence identity is invalid.");if(error instanceof CostEvidenceReadNotFound)return new HttpError(404,"cost_evidence_not_found","Cost evidence was not found.");if(error instanceof CostEvidenceReadPersistenceFailure)return new HttpError(500,"cost_evidence_read_failed","Cost evidence could not be read.");return new HttpError(500,"cost_evidence_read_failed","Cost evidence could not be read."); }
  private recipeCostHistoryOperation(error: unknown): HttpError { if(error instanceof RecipeCostHistoryReadValidationFailure)return new HttpError(422,"recipe_cost_history_invalid","Recipe Cost History identity is invalid.");if(error instanceof RecipeCostHistoryReadNotFound)return new HttpError(404,"recipe_cost_history_not_found","Recipe Cost History evidence was not found.");if(error instanceof RecipeCostHistoryReadPersistenceFailure)return new HttpError(500,"recipe_cost_history_read_failed","Recipe Cost History could not be read.");return new HttpError(500,"recipe_cost_history_read_failed","Recipe Cost History could not be read."); }
  private recipeCostAnalyticsOperation(error: unknown): HttpError { if(error instanceof RecipeCostAnalyticsValidationFailure)return new HttpError(422,"cost_analytics_invalid","Recipe Cost Analytics identity is invalid.");if(error instanceof RecipeCostAnalyticsReadFailure)return new HttpError(500,"cost_analytics_read_failed","Recipe Cost Analytics could not be read.");return new HttpError(500,"cost_analytics_read_failed","Recipe Cost Analytics could not be read."); }
