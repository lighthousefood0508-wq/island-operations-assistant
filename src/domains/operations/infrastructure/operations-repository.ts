...
import type { EventProduct, EventStatus, OperationsEvent, SellableInventory } from "../domain/types.js";
...
type InventoryRow = { event_id: string; product_id: string; product_version_id: string; planned_quantity: number; reserved_quantity: number; sold_quantity: number; created_at: string; updated_at: string };
...
function mapInventory(row: InventoryRow): SellableInventory {
  return { eventId: row.event_id, productId: row.product_id, productVersionId: row.product_version_id, plannedQuantity: row.planned_quantity, reservedQuantity: row.reserved_quantity, soldQuantity: row.sold_quantity, remainingQuantity: row.planned_quantity - row.reserved_quantity - row.sold_quantity, createdAt: row.created_at, updatedAt: row.updated_at };
}
...
export class OperationsRepository {
  ...
  listInventory(eventId: string): SellableInventory[] {
    return this.database.queryMany<InventoryRow>("SELECT event_id, product_id, product_version_id, planned_quantity, reserved_quantity, sold_quantity, created_at, updated_at FROM operations_sellable_inventory WHERE event_id = ? ORDER BY created_at", [eventId]).map(mapInventory);
  }
  setInventory(inventory: SellableInventory): void {
    this.database.execute(`INSERT INTO operations_sellable_inventory (event_id, product_id, product_version_id, planned_quantity, reserved_quantity, sold_quantity, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      ON CONFLICT(event_id, product_version_id) DO UPDATE SET planned_quantity = excluded.planned_quantity, updated_at = excluded.updated_at`,
      [inventory.eventId, inventory.productId, inventory.productVersionId, inventory.plannedQuantity, inventory.reservedQuantity, inventory.soldQuantity, inventory.createdAt, inventory.updatedAt]);
  }
