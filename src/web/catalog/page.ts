...
function renderProducts(){$('products').innerHTML=state.products.map(product=>'<div class="item product-row '+(product.productId===state.selectedProductId?'selected':'')+'">'+productSummary(product)+'<div class="actions"><button class="secondary" data-product="'+product.productId+'">編輯</button><button class="warn" data-deactivate-product="'+product.productId+'">停用</button></div></div>').join('')||'<div class="empty">目前沒有商品</div>'}
function fillProductForm(product){state.selectedProductId=product.productId;$('product-id').value=product.productId;$('internal-name').value=product.internalName;$('product-category').value=product.categoryId;$('display-name').value=product.draft.displayName||'';$('pos-name').value=product.draft.posName||'';$('selling-price').value=Number.isInteger(product.draft.sellingPrice)?String(product.draft.sellingPrice):'';$('description').value=product.draft.description||'';$('product-status').value=product.status;document.querySelectorAll('.channels input').forEach(input=>input.checked=product.draft.channels.includes(input.value));renderPublicationStatus();renderProducts()}
...
async function load(){[state.categories,state.products]=await Promise.all([api('/api/admin/categories'),api('/api/admin/products')]);renderCategories();const product=selectedProduct();if(state.selectedProductId&&!product)resetProduct();else if(product)fillProductForm(product);else renderPublicationStatus();renderProducts()}
...
document.addEventListener('click',async event=>{const target=event.target;if(!(target instanceof HTMLElement))return;const categoryButton=target.closest('[data-category]');const productButton=target.closest('[data-product]');const deactivateButton=target.closest('[data-deactivate-product]');try{if(categoryButton){const category=state.categories.find(item=>item.categoryId===categoryButton.dataset.category);if(!category)return;$('category-id').value=category.categoryId;$('category-code-display').value=category.code;$('category-name').value=category.displayName;$('category-sort').value=String(category.sortOrder);$('category-active').value=String(category.isActive);return}if(productButton){const product=state.products.find(item=>item.productId===productButton.dataset.product);if(product)fillProductForm(product);return}if(deactivateButton){const product=state.products.find(item=>item.productId===deactivateButton.dataset.deactivateProduct);if(!product)return;if(!confirm('停用 '+product.internalName+'？已發布版本會保留歷史，新場次不再使用此商品。'))return;await api('/api/admin/products/'+product.productId,{method:'PATCH',body:JSON.stringify({status:'inactive'})});await load();message('商品已停用')}}catch(error){message(error.message,true)}});
