...
type OrderRow = { order_id: string; order_number: string; event_id: string; source: string; created_at: string; customer_name: string | null; customer_phone_tail: string | null; payment_method: PaymentMethod | null; notes: string | null; order_status: OrderStatus; payment_status: PaymentStatus; production_status: ProductionStatus; cancellation_reason: string | null; grand_total: number; served_at: string | null };
...
export class LifecycleRepository {
  ...
  findOrder(orderId: string): LifecycleOrder | undefined {
    const row = this.database.queryOne<OrderRow>("SELECT order_id, order_number, event_id, source, created_at, customer_name, customer_phone_tail, payment_method, notes, order_status, payment_status, production_status, cancellation_reason, grand_total, served_at FROM operations_orders WHERE order_id = ?", [orderId]);
    return row && this.mapOrder(row);
  }
  listEventOrders(eventId: string): LifecycleOrder[] {
    return this.database.queryMany<OrderRow>("SELECT order_id, order_number, event_id, source, created_at, customer_name, customer_phone_tail, payment_method, notes, order_status, payment_status, production_status, cancellation_reason, grand_total, served_at FROM operations_orders WHERE event_id = ? ORDER BY created_at, order_number", [eventId]).map((row) => this.mapOrder(row));
  }
