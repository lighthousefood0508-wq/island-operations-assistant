  ...
  <style>
    ...
    *{box-sizing:border-box}body{margin:0}button{font:inherit}main{max-width:1600px;margin:auto;padding:10px 12px 22px}
    ...
    @media(max-width:900px){
      main{padding:9px}.top{grid-template-columns:minmax(0,1fr) auto;gap:8px}.top h1{font-size:27px}.top>.sync-status{grid-column:1/-1;grid-row:2}.metrics{grid-template-columns:repeat(3,minmax(0,1fr))}.metric{padding:8px}.metric strong{font-size:20px}
...
<body>
  ...
  <script>
    ...
    const state={event:null,orders:[],activeTab:'pending'};
    ...
    function clearForNoEvent(){state.event=null;state.orders=[];$('event').textContent='目前沒有營業中的場次';$('page-state').textContent='請先至後台開啟場次，Kitchen 才會顯示中央訂單。';$('page-state').className='page-state';render()}
    async function load(){try{const event=await api('/api/events/current');sync.setEventId(event?.eventId);if(!event){clearForNoEvent();return}const orders=await api('/api/events/'+event.eventId+'/orders');state.event=event;state.orders=orders;const status=String(event.status||'').toUpperCase();$('event').innerHTML=esc(event.displayName)+' · '+esc(event.eventCode)+' <span class="event-status '+esc(event.status)+'">'+esc(status)+'</span>';$('page-state').textContent=event.status==='paused'?'場次暫停中，Kitchen 僅處理現有訂單。':'中央訂單已同步。';$('page-state').className='page-state';render()}catch(error){$('page-state').textContent=(navigator.onLine?'中央資料讀取失敗：':'目前離線，無法讀取中央訂單：')+(error?.message||'未知錯誤');$('page-state').className='page-state error';throw error}}
    ...
    document.addEventListener('click',event=>{const tab=event.target.closest('[data-kitchen-tab]');if(tab){state.activeTab=tab.dataset.kitchenTab;renderTabs();return}const revert=event.target.closest('[data-revert-production]');if(revert){void revertCompletion(revert);return}const status=event.target.closest('[data-status]');if(status)void updateStatus(status)});
