...
<script>
const state={ingredients:[],profiles:[],recipes:[],products:[]};
...
async function loadQuotes(){const ingredientId=$('quote-ingredient').value;if(!ingredientId){$('quote-list').innerHTML='<span class="empty">選擇食材後顯示報價</span>';return}const quotes=await api('/api/admin/cost/quotes?ingredientId='+encodeURIComponent(ingredientId));$('quote-list').innerHTML=quotes.map(q=>'<div class="chip">'+esc(q.amount.coefficient)+' ×10^-'+esc(q.amount.scale)+' TWD / '+esc(q.purchaseQuantity.coefficient)+' '+esc(q.purchaseQuantity.unitCode)+' · '+esc(q.state)+'</div>').join('')||'<span class="empty">此食材尚無報價</span>'}
$('quote-ingredient').addEventListener('change',()=>loadQuotes().catch(error=>notify(error.message,true)));
$('evaluation-form').addEventListener('submit',async event=>{event.preventDefault();try{const outcome=await api('/api/admin/cost/evaluations',{method:'POST',body:JSON.stringify({recipeId:$('evaluation-recipe').value,evaluatedAt:$('evaluation-time').value})});if(outcome.status!=='evaluated'){$('evaluation-result').innerHTML='<div class="failure"><strong>'+esc(outcome.failure.code)+'</strong><br>'+esc(outcome.failure.message)+'</div>';return}const result=outcome.result;$('evaluation-result').innerHTML='<div class="result"><div class="metric"><small>標準批次成本 · '+esc(result.currencyCode)+'</small><strong>'+esc(result.exactStandardBatchCost.numerator)+' / '+esc(result.exactStandardBatchCost.denominator)+'</strong></div><div class="metric"><small>每標準份成本 · '+esc(result.currencyCode)+'</small><strong>'+esc(result.exactPerStandardYieldCost.numerator)+' / '+esc(result.exactPerStandardYieldCost.denominator)+'</strong></div></div><table class="trace"><thead><tr><th>行</th><th>食材</th><th>Quote</th><th>精確行成本</th></tr></thead><tbody>'+result.lines.map(line=>'<tr><td>'+esc(line.linePosition+1)+'</td><td>'+esc(state.ingredients.find(i=>i.ingredientId===line.ingredientId)?.name||line.ingredientId)+'</td><td>'+esc(line.quoteNormalizationEvidence.quoteId)+'</td><td>'+esc(line.exactLineCost.numerator)+' / '+esc(line.exactLineCost.denominator)+'</td></tr>').join('')+'</tbody></table>';notify('成本評估完成。')}catch(error){notify(error.message,true)}});
$('quote-effective').value=now();$('evaluation-time').value=$('quote-effective').value;
