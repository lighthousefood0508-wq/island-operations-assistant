...
Phase 1C-2 Order Lifecycle is complete on `feature/phase-1c2-order-lifecycle` under **DECISIONS #007**. New POS Orders are `pending`; legal backend transitions are `pending -> cooking -> ready -> completed`, with terminal `cancelled` and manual `no_show`. Inventory release is a separately confirmed, once-only action for no-show Orders. Formal Event Close blocks unresolved Orders, persists an idempotent daily-report snapshot, appends audit, and locks the Event. `/pos/lifecycle` is the minimal staff console for these actions.
