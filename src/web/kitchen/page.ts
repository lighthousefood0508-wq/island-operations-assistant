...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}body{margin:0}button{font:inherit}main{max-width:1600px;margin:auto;padding:10px 12px 22px}
    ...
    .order-list{display:grid;gap:10px}.card{border:1px solid #d9e4e0;border-radius:7px;background:#fbfdfc;padding:14px}.card.preparing{background:var(--hot);border-color:#e7cd9f}.card.ready,.card.served{background:var(--ready);border-color:#b9dec9}.card.locked{border:3px solid #b13f2e;background:#fff2ee}.card-top{display:flex;justify-content:space-between;gap:10px;align-items:flex-start}.order-no{font-size:31px;font-weight:900;color:#123f45}.timing{font-size:14px;color:#a33d25;font-weight:900;text-align:right}.customer{font-size:17px;font-weight:900;margin-top:5px}.items{font-size:25px;font-weight:900;margin:11px 0}.items div{margin:7px 0}.item-quantity{color:#a52d1f;font-size:1.12em}.meta{display:flex;flex-wrap:wrap;gap:7px;margin:8px 0}.tag{font-size:13px;background:#eef3f0;border-radius:999px;padding:5px 8px;color:#36575b;font-weight:800}.note{color:#7a4b16;font-weight:800;border-left:3px solid #d7a14a;padding-left:8px}.modification-lock{background:#b13f2e;color:#fff;border-radius:6px;padding:11px;font-weight:900;margin:10px 0}.change-list{border:1px solid #e4c5bb;background:#fff8f5;border-radius:6px;padding:9px;margin:9px 0}.change-list strong{display:block;margin-bottom:5px}.change-added{color:#08796b}.change-removed{color:#a52d1f}.actions{margin-top:12px}.actions button{width:100%;min-height:48px;border:0;border-radius:5px;background:#0f7668;color:#fff;font-weight:900;font-size:18px;cursor:pointer}.actions button:disabled{opacity:.55;cursor:not-allowed}.empty{color:var(--muted);font-weight:800;padding:12px 2px}
...
<body>
  ...
  <script>
    ...
    function readReminderLedger(){try{const saved=JSON.parse(localStorage.getItem(REMINDER_LEDGER_KEY)||'[]');return Array.isArray(saved)?saved.filter(item=>item&&typeof item.identity==='string'):[]}catch{return[]}}
    ...
    function ledgerIdentity(order,eventType){return [state.event?.eventId,pickupNumber(order),order.presentation?.effectiveRevision||1,eventType,eventType.startsWith('pickup-')?order.scheduledPickupAt||'':''].join(':')}
    function ledgerHas(order,eventType){const identity=ledgerIdentity(order,eventType);return state.reminderLedger.some(item=>item.identity===identity)}
    function ledgerAdd(order,eventType){const identity=ledgerIdentity(order,eventType);if(state.reminderLedger.some(item=>item.identity===identity))return;state.reminderLedger.push({identity});saveReminderLedger()}
    ...
    function pickupNumber(order){return order.presentation?.pickupNumber||order.orderNumber}
    function orderLabel(order){return pickupNumber(order)+(order.presentation?.modified?'（已修改）':'')}
    function orderVoice(order){const pickup=order.scheduledPickupAt?'取餐時間 '+timeLabel(order.scheduledPickupAt)+'，':'',name=order.customerName||'現場客',tail=order.customerPhoneTail?'，電話尾碼 '+window.__rosVoice.digits(order.customerPhoneTail):'';return '叫號 '+pickupNumber(order)+'，'+pickup+name+tail+'，'+window.__rosVoice.itemText(order.items)}
    ...
    function processAutomaticVoice(isBaseline=false){if(!state.event)return;const messages=[],now=Date.now(),lead=state.voiceSettings.reminderLeadMinutes*60000;for(const order of state.orders){const isActive=order.orderStatus==='confirmed'&&order.productionStatus!=='served',orderEvent=order.presentation?.modified?'order-modified':'new-order';if(!ledgerHas(order,orderEvent)){ledgerAdd(order,orderEvent);if(!isBaseline&&isActive&&state.voiceSettings.voiceEnabled&&state.voiceSettings.newOrderVoiceEnabled)messages.push(orderEvent==='order-modified'?'訂單 '+pickupNumber(order)+' 已修改':'新訂單，'+orderVoice(order))}if(!reminderEligible(order))continue;const pickup=new Date(order.scheduledPickupAt).getTime(),remaining=pickup-now;if(remaining>0&&remaining<=lead&&!ledgerHas(order,'pickup-before')){ledgerAdd(order,'pickup-before');if(!isBaseline&&state.voiceSettings.voiceEnabled&&state.voiceSettings.dueReminderEnabled)messages.push('預約提前提醒，'+orderVoice(order))}if(remaining<=0&&remaining>=-5*60000&&!ledgerHas(order,'pickup-due')){ledgerAdd(order,'pickup-due');if(!isBaseline&&state.voiceSettings.voiceEnabled&&state.voiceSettings.dueReminderEnabled)messages.push('預約到點提醒，'+orderVoice(order))}}if(messages.length)speak(messages.join('。'))}
    ...
    function workCard(order){const pickup=timeLabel(order.scheduledPickupAt),timing=pickup?'預約 '+esc(pickup):'等待 '+waitMinutes(order.createdAt)+' 分鐘',locked=Boolean(order.modification?.locked);let action='';if(!locked&&(order.productionStatus==='not_started'||order.productionStatus==='queued'))action='<button data-id="'+order.orderId+'" data-status="preparing">開始製作</button>';if(!locked&&order.productionStatus==='preparing')action='<button data-id="'+order.orderId+'" data-status="ready">完成製作</button>';if(!locked&&order.productionStatus==='ready')action='<button data-id="'+order.orderId+'" data-status="served">完成出餐</button>';if(locked)action='<button type="button" disabled>改單處理中，暫停狀態操作</button>';const changes=(order.modification?.lastChanges||[]).map(change=>'<div class="'+(change.kind==='removed'||change.afterQuantity<change.beforeQuantity?'change-removed':'change-added')+'">'+(change.kind==='added'?'新增：':change.kind==='removed'?'刪除：':change.kind==='note'?'備註修改：':'數量修改：')+esc(change.posName)+(change.kind==='note'?'':' ×'+Math.abs(change.afterQuantity-change.beforeQuantity))+'</div>').join('');return '<article class="card '+esc(order.productionStatus)+(locked?' locked':'')+'" data-order-id="'+order.orderId+'"><div class="card-top"><div><div class="order-no">'+esc(orderLabel(order))+'</div><div class="customer">'+(order.customerName?esc(order.customerName):'現場客')+(order.customerPhoneTail?' · 尾碼 '+esc(order.customerPhoneTail):'')+'</div></div><div class="timing" data-created-at="'+esc(order.createdAt)+'" data-scheduled-pickup-at="'+esc(order.scheduledPickupAt||'')+'">'+timing+'</div></div>'+(locked?'<div class="modification-lock">訂單 '+esc(pickupNumber(order))+' 改單處理中，暫停狀態操作</div>':'')+(changes?'<div class="change-list"><strong>本次修改</strong>'+changes+'</div>':'')+'<div class="items">'+orderItems(order)+'</div>'+(order.notes?'<div class="note">訂單備註：'+esc(order.notes)+'</div>':'')+'<div class="actions">'+action+'</div></article>'}
    function servedCard(order){const servedAt=timeLabel(order.servedAt),canRevert=order.orderStatus==='confirmed'&&order.productionStatus==='served';return '<article class="card served" data-order-id="'+order.orderId+'"><div class="card-top"><div><div class="order-no">'+esc(orderLabel(order))+'</div><div class="customer">'+(order.customerName?esc(order.customerName):'現場客')+(order.customerPhoneTail?' · 尾碼 '+esc(order.customerPhoneTail):'')+'</div></div><div class="timing">'+(servedAt?'出餐 '+esc(servedAt):'已出餐')+'</div></div><div class="items">'+orderItems(order)+'</div>'+(canRevert?'<div class="actions"><button type="button" data-revert-production="'+order.orderId+'">退回上一步</button></div>':'')+'</article>'}
