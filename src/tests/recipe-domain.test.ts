...
test("Recipe Draft rejects duplicate Ingredient identity and dimension mismatch", () => {
  const recipe = RecipeAggregate.createDraft({
    recipeId: RecipeId.fromUuid(UUIDS.recipe),
    draftId: RecipeDraftId.fromUuid(UUIDS.draft),
    name: "Dongpo Pork",
  ...
  recipe.addIngredient(pork, Quantity.create(600n, 0, gram));

  assert.throws(
    () => recipe.addIngredient(pork, Quantity.create(200n, 0, gram)),
    DuplicateIngredient
  );
  assert.throws(
    () => RecipeAggregate.createDraft({
      ...
      createdBy: "owner",
      createdAt: timestamp
    }).addIngredient(pork, Quantity.create(1n, 0, serving)),
    InvalidRecipeState
  );
});
