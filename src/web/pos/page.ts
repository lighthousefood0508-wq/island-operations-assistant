...
<head>
  ...
  <title>ROS POS</title>
  <style>
    :root{font-family:Arial,sans-serif;color:#163237;background:#f3f6f4}*{box-sizing:border-box}body{margin:0}main{max-width:1440px;margin:auto;padding:16px}${renderNavigationStyles()}.topbar{display:flex;align-items:center;justify-content:space-between;gap:16px;border-bottom:1px solid #cbd8d4;padding-bottom:12px}.topbar h1{font-size:25px;margin:0}.event{margin:4px 0 0;color:#587075;font-size:14px}.tabs{display:flex;gap:6px;overflow-x:auto;padding:14px 0 10px}.tab{white-space:nowrap;background:#e4ece8;color:#294b50;border:1px solid #c6d4cf;border-radius:4px;padding:9px 12px;font:inherit;cursor:pointer}.tab[aria-selected="true"]{background:#165b59;border-color:#165b59;color:#fff}.pane[hidden],#sync-debug[hidden]{display:none}.stats{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px;margin-bottom:12px}.stat,.panel,.product,.order{background:#fff;border:1px solid #cad6d1;border-radius:5px}.stat{padding:13px}.stat span{display:block;color:#587075;font-size:13px}.stat strong{display:block;font-size:22px;margin-top:4px}.layout{display:grid;grid-template-columns:minmax(0,1fr) 380px;gap:12px}.panel{padding:14px}.panel h2,.panel h3{margin:0 0 10px}.panel h2{font-size:19px}.category-tabs{display:flex;gap:7px;overflow-x:auto;margin-bottom:12px}.category-tab{background:#edf3f0;border:1px solid #cad6d1;color:#294b50;border-radius:4px;padding:7px 10px;font:inherit;white-space:nowrap;cursor:pointer}.category-tab[aria-pressed="true"]{background:#d7ebe5;border-color:#165b59;color:#123c3a}.products{display:grid;grid-template-columns:repeat(auto-fill,minmax(160px,1fr));gap:10px}.product{padding:12px;min-height:164px;display:flex;flex-direction:column}.product.sold-out{background:#eef1ef;border-color:#d5dcda;color:#71817e}.product-name{font-weight:700;font-size:17px}.product-meta{color:#587075;font-size:13px;margin-top:3px}.product-price{font-weight:700;margin:10px 0 3px}.product button{margin-top:auto}.soldout{color:#a52e1b;font-weight:700}.cart{position:sticky;top:58px;align-self:start}.cart-items{display:grid;gap:8px;max-height:38vh;overflow:auto}.cart-line{border-bottom:1px solid #dce5e1;padding-bottom:9px}.cart-main{display:grid;grid-template-columns:minmax(0,1fr) auto auto auto;gap:7px;align-items:center}.line-total{font-weight:700}.quantity{display:flex;align-items:center;gap:4px}.quantity button,.icon-button{min-width:30px;padding:5px 7px}.field{display:block;margin-top:10px;color:#3e5d61;font-size:14px}.field input,.field select,.field textarea{display:block;width:100%;font:inherit;border:1px solid #b9cbc4;border-radius:4px;padding:8px;margin-top:4px;background:#fff}.field textarea{min-height:58px;resize:vertical}.cart-footer{border-top:1px solid #cad6d1;margin-top:12px;padding-top:12px}.total-row{display:flex;align-items:center;justify-content:space-between;font-size:19px;font-weight:700;margin:10px 0}.primary,button{font:inherit;border:0;border-radius:4px;background:#165b59;color:#fff;padding:9px 12px;cursor:pointer}.primary{width:100%;font-size:16px}.secondary{background:#617477}.danger{background:#a52e1b}button:disabled{cursor:not-allowed;opacity:.52}.notice{min-height:26px;margin:10px 0 0;color:#a52e1b;display:flex;gap:8px;align-items:center}.notice button{padding:4px 8px;background:#617477}.order-list{display:grid;gap:8px}.served-section{margin-top:18px;padding-top:14px;border-top:1px solid #dce5e1}.order{padding:11px}.order-top{display:flex;justify-content:space-between;gap:10px}.order-number{font-weight:700}.order-meta,.order-items{font-size:13px;color:#587075;margin-top:5px}.status-list{display:flex;gap:5px;flex-wrap:wrap;margin-top:7px}.status{font-size:12px;padding:3px 6px;border-radius:999px;background:#edf3f0;color:#36575b}.empty{color:#587075;padding:12px 0}.unavailable{max-width:640px;color:#587075}@media(max-width:900px){main{padding:12px}.layout{grid-template-columns:1fr}.cart{position:static}.products{grid-template-columns:repeat(auto-fill,minmax(145px,1fr))}}@media(max-width:620px){.topbar{align-items:flex-start;flex-direction:column}.stats{grid-template-columns:repeat(2,minmax(0,1fr))}.cart-main{grid-template-columns:minmax(0,1fr) auto auto}.line-total{grid-column:1/-1}.product{min-height:152px}}
  </style>
</head>
<body>
  ${renderSystemNav("pos")}
    ...
    <section class="pane" data-pane="onsite">
      ...
      <div class="layout">
        ...
        <aside class="panel cart">
          ...
          <label class="field">客人稱呼<input id="customer-name" maxlength="80" autocomplete="off" placeholder="可不填"></label>
          <label class="field">電話末四碼<input id="customer-phone-tail" maxlength="4" inputmode="numeric" autocomplete="off" placeholder="例如 1234"></label>
          <label class="field">付款方式<select id="payment-method"><option value="">請選擇</option><option value="CASH">現金</option><option value="LINE_PAY">LINE Pay</option></select></label>
          <label class="field">訂單備註<textarea id="order-notes" maxlength="500" placeholder="可不填"></textarea></label>
          <div class="cart-footer">
            <button class="secondary" id="clear-cart" type="button">清空購物車</button>
            <div class="total-row"><span>總計</span><strong id="total">NT$0</strong></div>
            <button id="create-order" class="primary">建立中央訂單</button>
  ...
  <script>
    ...
    function renderCart(){const total=[...s.cart.values()].reduce((sum,item)=>sum+item.sellingPrice*item.quantity,0);$('total').textContent=money(total);$('cart-items').innerHTML=[...s.cart.values()].map(item=>'<article class="cart-line"><div class="cart-main"><strong>'+escape(item.posName)+'</strong><div class="quantity"><button type="button" data-adjust="-1" data-product-id="'+item.productId+'">-</button><span>'+item.quantity+'</span><button type="button" data-adjust="1" data-product-id="'+item.productId+'" '+(item.quantity>=item.remainingQuantity?'disabled':'')+'>+</button></div><strong class="line-total">'+money(item.quantity*item.sellingPrice)+'</strong><button type="button" class="danger icon-button" data-cart="'+item.productId+'" aria-label="刪除 '+escape(item.posName)+'">刪除</button></div><label class="field">品項備註<input data-note="'+item.productId+'" maxlength="250" value="'+escape(item.note||'')+'" placeholder="可不填"></label></article>').join('')||'<p class="empty">尚未加入商品</p>';$('create-order').disabled=s.busy||!s.cart.size;$('clear-cart').disabled=s.busy||!s.cart.size}
