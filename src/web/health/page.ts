...
function renderLinks(){const origin=location.origin,links=[['ROS Base','/'],['POS 點餐','/pos'],['廚房系統','/kitchen'],['後台管理','/admin'],['商品目錄','/admin/catalog'],['場次與備貨','/admin'],['今日統計','/admin/statistics'],['系統狀態','/admin/health'],['裝置連線','/admin/devices']];$('links').innerHTML=links.map(([label,path])=>'<div class="share-row"><code>'+origin+path+'</code><button type="button" data-copy="'+origin+path+'">複製 '+label+'</button></div>').join('')}
